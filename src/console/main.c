...
int main(void)
{
    ...
    const UmiExperienceLayoutDefinition *layout =
        umi_education_workspace_default();
    const UmiExperienceFeatureDefinition *next =
        umi_education_workspace_next_feature();
     ...
     */
    if (definition == NULL ||
        umi_education_application_status(&status) != UMI_STATUS_OK ||
        layout == NULL) {
        fputs("Umicom Education Studio: Framework experience unavailable\n", stderr);
        return 1;
    }
