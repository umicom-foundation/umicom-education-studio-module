...
int main(void)
{
    ...
    assert(umi_education_runtime_init(&runtime) == UMI_STATUS_OK);
    assert(umi_education_workspace_select_layout(
               &runtime, experience->default_layout_id) == UMI_STATUS_OK);
    assert(umi_education_workspace_activate_panel(
               &runtime, experience->panels[0].panel_id) == UMI_STATUS_OK);
    commands = umi_education_workspace_commands(&runtime);
    ...
    assert(umi_education_workspace_commands(NULL) == NULL);
    return 0;
}
