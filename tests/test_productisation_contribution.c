...
int main(void)
{
    ...
    assert(strcmp(adoption->application_id, "org.umicom.education") == 0);
    assert(umi_product_application_adoption_validate(adoption) ==
           UMI_STATUS_OK);
    assert(umi_education_productisation_snapshot(&snapshot) ==
           UMI_STATUS_OK);
    ...
    assert(umi_education_product_session_init(&session) ==
           UMI_STATUS_OK);
    ...
    assert(umi_education_product_workspace_guide(&workspace_guide) ==
           UMI_STATUS_OK);
